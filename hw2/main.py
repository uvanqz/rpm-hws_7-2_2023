    ...
    def make_changes(self):
        if self.path.startswith(EVENTS):
            if self.command == 'POST':
                ...
                try:
                    body, query = self.get_body(), self.get_query(EVENTS_ALL_ATTRS)
                except Exception as error:
                    code, msg = BAD_REQUEST, str(error)
                else:
                    code, msg = (OK, 'OK') if db_update(EVENTS[1:], query, body) else (BAD_REQUEST, 'ERR')
                ...
                try:
                    ...
                    code, msg = (OK, 'OK') if db_delete(EVENTS[1:], query) else (NOT_FOUND, 'FAIL')
